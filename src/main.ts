import './styles.css';
import { clearData, loadData, saveData, updateData, validateImport, type StorageNamespace } from './db';
...
import { DEFAULT_ALIASES, DEFAULT_DATA, DEMO_DATA, type AppData, type LiftSet, type Workout } from './types';
...
let restState: 'idle' | 'running' | 'done' = 'idle';
let isPro = false;
...
let checkoutState: 'unknown' | 'checking' | 'available' | 'unavailable' = 'unknown';
...
function shell(content: string, page: 'app' | 'legal' = 'app'): string {
  ...
  ${content}
  <footer><p>Workout data stays in this browser. Pro checks use Sociobot.</p><nav aria-label="Legal"><a href="/privacy" data-route="privacy">Privacy</a><a href="/terms" data-route="terms">Terms</a></nav><p class="disclosure">Built by Param Factory · v1.0.1 · Generated editorial image.</p></footer>
...
function renderLegal(kind: 'privacy' | 'terms'): void {
  const privacy = `<main id="main" class="legal"><p class="eyebrow">WORKOUT AND LICENSE DATA</p><h1>Privacy</h1>
    ...
    <h2>What leaves your browser</h2><p>Opening Setup checks whether Sociobot checkout is available. This sends no workout data. If you use Pro, your license token is also checked there.</p>
  ...
  const terms = `<main id="main" class="legal"><p class="eyebrow">USE AND PRO LICENSE TERMS</p><h1>Terms</h1>
    ...
    <h2>License and purchase</h2><p>The free logger remains useful without payment. Set Receipt Pro is a $9 one-time purchase for custom rest intervals and private receipt notes when a valid license is present.</p><p>Sociobot/Dodo is the merchant of record. Refunds are handled there and automatically revoke the Pro license.</p>
...
function logView(): string {
  ...
  const activeVolume = workout ? workoutVolume(workout) : null;
  ...
  return `<main id="main" class="log-layout">
    ...
    <aside class="utility-rail" aria-label="Workout utilities">
      <section class="rest-block"><p class="eyebrow">REST TIMER</p><div id="rest-time" class="rest-time" role="timer">${restState === 'done' ? 'DONE' : restState === 'running' ? restDisplay() : formatClock(data.settings.restSeconds)}</div><div class="timer-actions"><button data-action="timer-toggle">${restState === 'running' ? 'Pause rest timer' : 'Start rest timer'}</button><button data-action="timer-reset">Reset rest timer</button></div><p>${Math.round(data.settings.restSeconds / 60)} min default · starts after each set</p></section>
...
function landingSections(): string {
  return `<section class="landing-sections" aria-label="Set Receipt details">
    ...
    <section class="landing-pro"><p class="eyebrow">PRO FEATURES</p><h2>Set Receipt Pro: $9 once</h2><p>Pro adds custom rest intervals and private notes on finished receipts.</p><button class="primary-button" data-view="settings">See Pro purchase</button><p><a href="/privacy" data-route="privacy">Privacy</a> · <a href="/terms" data-route="terms">Terms</a></p></section>
  </section>`;
}
...
function receiptMarkup(workout: Workout, compact = false): string {
  ...
  return `<article class="receipt ${compact ? 'compact' : ''}" aria-label="Workout receipt for ${escapeHtml(dateLabel(workout.startedAt))}">
    ...
    <dl class="totals"><div><dt>Sets</dt><dd>${workout.sets.length}</dd></div><div><dt>${volume.label}</dt><dd>${volume.text}</dd></div><div><dt>Duration</dt><dd>${formatDuration(workoutDuration(workout))}</dd></div><div><dt>PRs</dt><dd>${prs}</dd></div></dl>
...
function historyView(): string {
  ...
  return `<main id="main" class="history-page"><div class="page-heading"><div><p class="eyebrow">FINISHED WORKOUTS</p><h1>Workout receipts</h1></div><span class="big-count">${receipts.length.toString().padStart(2, '0')}</span></div>
    ${receipts.length ? `<div class="receipt-grid">${receipts.map((workout) => { const volume = workoutVolume(workout); return `<button class="receipt-card" data-receipt="${escapeHtml(workout.id)}"><span>${dateLabel(workout.startedAt)}</span><strong>${workout.sets.length} sets</strong><span>${volume.label}: ${volume.text}</span><i>View receipt →</i></button>`; }).join('')}</div>` : `<section class="history-empty"><img src="/assets/set-receipt-hero.webp" width="800" height="800" decoding="async" alt="A blank paper receipt curling across blue weight plates beside an orange barbell collar"><div><p class="eyebrow">NO RECEIPTS YET</p><h2>Finish a workout to file it here.</h2><p>Your active sets are safe in the Log tab.</p><button class="primary-button" data-view="log">Log a set</button></div></section>`}
  </main>`;
}
...
function render(): void {
  ...
  makeHeadingFocusable();
  updateTimer();
  if (view === 'settings' && !demoMode && !isPro && checkoutState === 'unknown') void checkCheckoutAvailability();
}
...
async function checkCheckoutAvailability(): Promise<void> {
  ...
  try {
    ...
    const body = await response.json() as { data?: Array<{ slug?: string; price_minor?: number; checkout_url?: string }> };
    const product = body.data?.find((item) => item.slug === 'lift-receipt-log');
    checkoutState = product?.price_minor === 900 && product.checkout_url === CHECKOUT_URL ? 'available' : 'unavailable';
  } catch {
...
function updateTimer(): void {
  ...
  if (restState !== 'running') {
    output.textContent = formatClock(data.settings.restSeconds);
    return;
  }
  output.textContent = restDisplay();
...
function resetTimer(state: 'idle' | 'done' = 'idle'): void {
  restEndsAt = 0;
  restState = state;
  window.clearInterval(restTimer);
}

async function persistChange(change: (latest: AppData) => void, message = ''): Promise<void> {
  try {
    data = await updateData(change, storageNamespace);
    storageFailed = false;
    if (message) showStatus(message);
  } catch {
    const tabCopy = structuredClone(data);
    change(tabCopy);
    data = tabCopy;
    storageFailed = true;
...
async function replaceData(next: AppData, message = ''): Promise<void> {
  try {
    await saveData(next, storageNamespace);
    data = next;
    storageFailed = false;
    if (message) showStatus(message);
  } catch {
...
function isPersonalRecord(source: AppData, exercise: string, weight: number, unit: 'lb' | 'kg'): boolean {
  const kg = unit === 'kg' ? weight : weight * 0.453592;
  const prior = source.workouts.flatMap((workout) => workout.sets).filter((set) => set.exercise.toLowerCase() === exercise.toLowerCase());
...
async function addSet(form: HTMLFormElement): Promise<void> {
  ...
  try {
    ...
    await persistChange((latest) => {
      let workout = latest.workouts.find((item) => !item.endedAt);
      if (!workout) {
        workout = { id: id(), startedAt: createdAt, endedAt: null, sets: [] };
        latest.workouts.push(workout);
      }
      workout.sets.push({ id: setId, exercise, ...parsed, createdAt, isPr: isPersonalRecord(latest, exercise, parsed.weight, parsed.unit) });
    }, `${exercise} ${parsed.weight}${parsed.unit} × ${parsed.reps} logged.`);
    startTimer();
    render();
...
async function finishWorkout(): Promise<void> {
  ...
  await persistChange((latest) => {
    const current = latest.workouts.find((item) => item.id === workout.id);
    if (current?.sets.length) current.endedAt = endedAt;
  }, 'Workout finished. Receipt filed.');
...
app.addEventListener('submit', async (event) => {
  ...
  if (form.id === 'alias-form') {
    ...
    const newAlias = { id: id(), alias, exercise: canonicalExercise(exercise, []) };
    let added = false;
    await persistChange((latest) => {
      if (latest.aliases.some((item) => item.alias.toLowerCase() === alias)) return;
      latest.aliases.push(newAlias);
      added = true;
    });
    if (added) showStatus('Alias added.');
    else showError('That short code already exists.');
    render();
  }
  if (form.id === 'custom-rest-form') {
    ...
    await persistChange((latest) => { latest.settings.restSeconds = seconds; }, 'Custom rest saved.'); render();
  }
...
app.addEventListener('change', async (event) => {
  ...
  if (target.name === 'unit') { const unit = target.value as 'lb' | 'kg'; await persistChange((latest) => { latest.settings.unit = unit; }, 'Default unit saved.'); render(); }
  if (target.dataset.setting === 'rest') { const seconds = Number(target.value); resetTimer(); await persistChange((latest) => { latest.settings.restSeconds = seconds; }, 'Rest timer saved.'); render(); }
  if (target.dataset.note && isPro) { const workoutId = target.dataset.note; const note = target.value.trim(); await persistChange((latest) => { const workout = latest.workouts.find((item) => item.id === workoutId); if (workout) workout.note = note; }, 'Receipt note saved.'); }
  if (target.id === 'import-file' && target instanceof HTMLInputElement && target.files?.[0]) {
    try {
      ...
      await replaceData(imported, 'Backup imported.'); render();
...
app.addEventListener('click', async (event) => {
  ...
  if (target.dataset.deleteSet) {
    ...
    if (workout && index >= 0) {
      undoSet = { workoutId: workout.id, set: workout.sets[index], index };
      const workoutId = workout.id;
      const setId = workout.sets[index].id;
      await persistChange((latest) => { const current = latest.workouts.find((item) => item.id === workoutId); if (current) current.sets = current.sets.filter((set) => set.id !== setId); }, 'Set removed.');
      window.clearTimeout(undoTimeout);
      undoTimeout = window.setTimeout(() => { undoSet = null; render(); }, 6000);
      render();
    }
    return;
  }
  if (target.dataset.deleteAlias) { const aliasId = target.dataset.deleteAlias; await persistChange((latest) => { latest.aliases = latest.aliases.filter((item) => item.id !== aliasId); }, 'Alias removed.'); render(); return; }
  ...
  switch (target.dataset.action) {
    ...
    case 'timer-toggle': restState === 'running' ? resetTimer() : startTimer(); render(); break;
    case 'timer-reset': resetTimer(); render(); break;
    case 'undo-delete': {
      if (!undoSet) break;
      const pending = undoSet;
      undoSet = null;
      await persistChange((latest) => {
        const workout = latest.workouts.find((item) => item.id === pending.workoutId);
        if (workout && !workout.sets.some((set) => set.id === pending.set.id)) workout.sets.splice(Math.min(pending.index, workout.sets.length), 0, pending.set);
      }, 'Set restored.');
      render();
      break;
    }
    case 'export-json': data = await loadData(storageNamespace); download(`set-receipt-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(data, null, 2), 'application/json'); showStatus('JSON backup exported.'); render(); break;
    case 'export-csv': data = await loadData(storageNamespace); download(`set-receipt-${new Date().toISOString().slice(0, 10)}.csv`, csvText(data.workouts), 'text/csv'); showStatus('CSV exported.'); render(); break;
    case 'erase-data':
      if (confirm(`Erase ${data.workouts.length} workout(s), all aliases, and settings from this browser? Export first if you need a backup.`)) {
        const emptyData = structuredClone(DEFAULT_DATA);
        selectedReceipt = null;
        resetTimer();
        undoSet = null;
        window.clearTimeout(undoTimeout);
        await replaceData(emptyData, 'All local workout data erased.');
        view = 'log';
        render();
      }
      break;
    case 'reset-demo':
      if (!demoMode) break;
      const sampleData = structuredClone(DEMO_DATA);
      selectedReceipt = null;
      view = 'log';
      resetTimer();
      undoSet = null;
      window.clearTimeout(undoTimeout);
      await replaceData(sampleData, 'Demo reset to sample data.');
      render();
      break;
    case 'exit-demo':
      if (!demoMode) break;
